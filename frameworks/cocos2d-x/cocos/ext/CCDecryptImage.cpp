...
#pragma pack(pop)
	...
	{
        std::string::size_type posSlash = file_path.rfind('/');
        std::string fileName;
        std::string fileDir;
        if (std::string::npos != posSlash)
        {
            fileName = file_path.substr(posSlash+1);
            fileDir = file_path.substr(0, posSlash+1);
        }
        else
        {
            fileName = file_path;
        ...
        {
            text[0] = fileDir + fileName.substr(0, pos);
